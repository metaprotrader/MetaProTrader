...
O MetaPro Trader reúne gráficos, leitura de fluxo, envio de ordens e análise assistida por inteligência artificial em um único aplicativo, disponível para Windows, Linux e macOS. A conexão com os servidores MetaTrader 5 é nativa, então você mantém vários servidores conectados ao mesmo tempo sem deixar o terminal do MetaTrader aberto.

As ordens saem da sua máquina direto para o servidor MT5 da corretora. Elas não passam pela infraestrutura da MetaPro Trader.

- 🌐 **Site:** [metaprotrader.com.br](https://www.metaprotrader.com.br)
- 📘 **Manual:** [Documentação on-line](https://www.metaprotrader.com.br/documentation)
- ⬇️ **Downloads:** [Versão mais recente](../../releases/latest)

---

## Download

Cada versão publica quatro builds:

| Plataforma | Arquivo | Observação |
|---|---|---|
| Windows (instalador) | `*win-Setup.exe` | Instalação guiada para Windows 10 e 11 |
| Windows (portátil) | `*win-Portable.zip` | Roda sem instalar |
| Linux | `*.AppImage` | Executável único |
| macOS | `*osx-Portable.zip` | Pacote portátil |

Escolha o arquivo do seu sistema na [página de versões](../../releases/latest) ou use a [página de download](https://www.metaprotrader.com.br/download) do site, que sempre aponta para a versão mais recente.

## Requisitos

- **Sistema operacional:** Windows 10 ou 11, macOS ou Linux
- **Processador:** Intel, AMD ou Apple Silicon de geração atual
- **Memória:** 8 GB de RAM ou mais
- **Aceleração por GPU:** disponível no Windows; no macOS e no Linux a execução é por CPU
- **Terminal MetaTrader 5:** não é necessário, porque o aplicativo fala direto com o servidor MT5 da sua corretora
- **Conta:** assinatura ativa da MetaPro Trader, validada no login

Quantos servidores MT5 você consegue manter conectados ao mesmo tempo depende do seu hardware: memória, processador, disco e banda de internet.

## Primeiros passos

1. **Instale e abra** o arquivo correspondente ao seu sistema.
2. **Faça login** com seu usuário e senha da MetaPro Trader.
3. **Adicione uma conexão** em *Servidores MetaTrader 5*, buscando pelo nome da corretora ou informando domínio (ou IP) e porta. Preencha a conta, a senha e um nome para a conexão.
4. **Abra um gráfico** a partir de um servidor conectado, em *Novo gráfico*.
5. **Monte sua área de trabalho**: escolha o tempo gráfico, adicione indicadores e salve um template.
6. **Ative a IA no gráfico** ou abra o *Laboratório de Estratégias* quando quiser análise assistida.

O passo a passo completo, com telas, está no [manual on-line](https://www.metaprotrader.com.br/documentation).

## Recursos

### Conexão com a corretora

- Conexão nativa com servidores MetaTrader 5, sem instalar o terminal MT5
- Vários servidores de corretoras conectados ao mesmo tempo, cada um com seus gráficos
- Cadastro do servidor por busca da corretora ou por domínio/IP e porta
- Conexão automática ao iniciar o aplicativo, se você quiser
- Ordens enviadas da sua máquina direto para o servidor MT5 da corretora

### Leitura de fluxo e profundidade

- **Footprint**, célula a célula, com volume e delta por preço
- **Delta acumulado (CVD)**, com identificação de divergências
- **Perfil da sessão** e indicadores de sessão no cabeçalho
- **Livro de ofertas**
- **Times and Sales** com filtro rápido, janela de filtros por regras e medidor de agressão
- Sobreposição da sua posição no fluxo

### Gráficos e análise técnica

- Mais de 50 indicadores técnicos personalizáveis
- Ferramentas de desenho, Fibonacci e anotações em texto
- Templates de indicadores para salvar e recarregar
- Configurações de gráfico por área de trabalho

### Análise assistida por IA

- Análise de padrões no gráfico, com as leituras que você mesmo configura
- **Painel de consenso**, com direção e confiança, além das seções ao vivo e de desempenho
- **Treinamento e retreinamento da IA** dentro do aplicativo, com os dados e parâmetros que você escolher
- **Laboratório de Estratégias (Robot Entry Lab):** automação de entradas a partir das previsões da IA, com escolha de estratégia, tipo de entrada, trailing stop, cartões de resultado e tabela de análise

### Negociação e risco

- Ordens a mercado, ordens limitadas e encerramento de posições
- Histórico de ordens
- Gerenciamento de risco
- Situação da assinatura visível dentro do aplicativo

### Aplicativo

- Windows, Linux e macOS a partir da mesma base de código
- Interface em português (Brasil) e inglês
- Verificação de atualizações integrada
- Restauração das configurações de fábrica quando precisar começar do zero

## Documentação

O manual cobre o aplicativo inteiro e está publicado em português e inglês:

- Login, tela principal e servidores MetaTrader 5
- Gráfico, barra de ferramentas, indicadores e templates
- Livro de ofertas, Times and Sales e as regras de filtro
- Inteligência artificial no gráfico, painel de consenso e treinamento da IA
- Menu de negociação, envio de ordens e lista de ordens
- Configurações, Fibonacci e situação da assinatura
- Ajuda, atualizações e manutenção
- Robot Entry Lab e o card MetaPro Trader IA
- Order Flow: Footprint, delta e profile

Leia em [metaprotrader.com.br/documentation](https://www.metaprotrader.com.br/documentation) ou abra *Ajuda → Manual on-line* dentro do aplicativo.

## Assinatura

O MetaPro Trader é um aplicativo pago. O plano **Pro** libera todos os recursos acima, nos dois ciclos de cobrança:

| Ciclo | Brasil (BRL) | Internacional (USD) |
|---|---|---|
| Mensal | R$ 99,90 / mês | US$ 19,90 / mês |
| Anual | R$ 999,00 / ano | US$ 199,00 / ano |

- **Garantia de 7 dias**, com reembolso integral, conforme o direito de arrependimento do art. 49 do Código de Defesa do Consumidor
- **Cancelamento a qualquer momento**
- A licença é por usuário: você pode instalar no Windows, no Linux ou no macOS, com **uma conexão simultânea por usuário**

Assine em [metaprotrader.com.br](https://www.metaprotrader.com.br).
...
- **Verificação de compatibilidade:** busque sua corretora na [página de verificação](https://www.metaprotrader.com.br/broker-check)
- **Programa de certificação:** a corretora passa por mapeamento técnico, testes controlados de conectividade e validação de estabilidade, e recebe o selo *Corretora certificada*
- **White-label:** a plataforma pode levar o logotipo, os textos, o nome e o domínio da sua corretora. Detalhes na [página para corretoras](https://www.metaprotrader.com.br/metapro-para-corretoras)

## Como relatar um problema

Abra um chamado pela sua conta ou escreva para suporte@metaprotrader.com.br, informando:

- a versão instalada, que aparece em *Ajuda → Sobre*
- seu sistema operacional
- a corretora e o servidor em que estava conectado
- o que você esperava que acontecesse e o que aconteceu

## Aviso de risco

O MetaPro Trader é um software de análise e execução. Ele não faz recomendação de investimento, e nenhum indicador, leitura de IA ou entrada automatizada é capaz de prever o comportamento do mercado. Operar instrumentos alavancados envolve o risco de perdas superiores ao capital investido. Você continua responsável por todas as ordens enviadas na sua conta, inclusive pelas ordens enviadas por automação. Teste sua configuração em conta demo antes de operar com dinheiro real.
